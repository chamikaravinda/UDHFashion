...
public class BankController {
	...
	public ModelAndView AddBanks(ModelAndView model, @ModelAttribute("bankAccount") BankAccount account,
	...
	public ModelAndView ShowEditBanks(ModelAndView model, @ModelAttribute("bankAccount") BankAccount account,
			RedirectAttributes redir) {
	...
	public ModelAndView deletBankAccount(ModelAndView model, @RequestParam("id") int id,RedirectAttributes redir) {

		if (serviceBank.RemoveAccount(id)) {
	...
	public ModelAndView AddDeposit(ModelAndView model, @ModelAttribute("bankDeposit") BankDeposites deposites,
			RedirectAttributes redir) {
	...
	public ModelAndView AddWithdraws(ModelAndView model, @ModelAttribute("bankWithdraw") BankWithdraws withdraw,
			RedirectAttributes redir) {

		if (serviceBank.AddBanKWithdraws(withdraw)) {
			if (serviceBank.UpdateBankBalanceAfterWithdraw(withdraw)) {
				redir.addFlashAttribute("success", 1);
				model.setViewName("redirect:/allWithdraws");
				return model;
			} else {

				model.addObject("error", "Adding deposite was unsuccesful");
				model.setViewName("redirect:/addWithdraw");
				return model;
