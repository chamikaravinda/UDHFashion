...
public class SalesInvoicePDF extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		ArrayList<Billitems> billItems = (ArrayList<Billitems>) model.get("billItems");
		...
		Paragraph invoiceId = new Paragraph("Invoice No :" + bill.getId());
		invoiceId.setFont(PdfFontFactory.createFont(FontConstants.HELVETICA));
		invoiceId.setFontSize(6f);
		invoiceId.setHorizontalAlignment(HorizontalAlignment.CENTER);
		invoiceId.setTextAlignment(TextAlignment.CENTER);
		invoiceId.setFixedLeading(6);

		Paragraph para2 = new Paragraph("Date : " + bill.getDate());
...
;
		...
		try {
			...
			for (Billitems cart : billItems) {
				...
				p3.add(formatter.format(cart.getPrice()-cart.getReduseDiscount())+ "");
				p3.setFontSize(6f);
			...
			Paragraph p4 = new Paragraph(formatter.format(bill.getNetAmount()));
			p4.setFontSize(7f);
			p4.setBold();
			...
			Paragraph payedAmount = new Paragraph("Paid Amount Rs : " + formatter.format(bill.getCash()));
			payedAmount.setFontSize(6f);
			payedAmount.setHorizontalAlignment(HorizontalAlignment.CENTER);
			payedAmount.setTextAlignment(TextAlignment.CENTER);
			payedAmount.setFixedLeading(6);

			Paragraph balance = new Paragraph("Balance Rs : " + formatter.format(bill.getBalance()));
			balance.setFontSize(6f);
			balance.setHorizontalAlignment(HorizontalAlignment.CENTER);
			balance.setTextAlignment(TextAlignment.CENTER);
			balance.setFixedLeading(6);
			
			Paragraph exchange = new Paragraph("Exchange valid within 3 days");
			exchange.setFontSize(6f);
			exchange.setBold();
